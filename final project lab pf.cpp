...
void fun1(){
	cout << setw(59) << "Welcome to the dining of the hotel!" << endl;
			...
			while (true) {
				...
				if (selected_cuisine >= 1 && selected_cuisine <= CUISINES) {
					selected_cuisine--;
					...
					cout << "No Cuisine Found! Exiting System....\n";
					break;
				}

				char choice;
				cout << "Do you want to select more dishes? (y for /no for any variable): ";
				cin >> choice;
			...
			ofstream outputFile;
			outputFile.open("E:\\Shumail.txt", ios::app);
\
//			cout << "Enter your name: "<<endl<<"Enter your adress:"<<endl<<"Enter your number:"<<endl;
//			cin.ignore();
...
//			cin.ignore();
			...
			// Write the total bill to the file
			outputFile << "**************\n";
			outputFile << "Total Bill: $" << total_bill << endl;
			outputFile << "Name is :" << name << endl;
			outputFile << "Adress is :" << adress << endl;
            outputFile << "Number is :" << number << endl;
			// Close the file
			outputFile.close();
...
int main()
{
	do{
		for(int i=0;i<80;i++)
				...
				cout<<"1. "<<a <<endl<<"2. "<<b <<endl<<"3. "<<c <<endl<<"4. "<<d <<endl<<"5. "<<e <<endl<<"6. "<<f <<endl<<"7. "<<g;
				cout<<endl;
				for(int i=0;i<80;i++)
				{
					cout<<"*";
